...
function [new_img, new_M] = affine(old_img, old_M, new_elem_size, verbose, bg, method)
   ...
   %
   XYZmm = old_R*(XYZvox-1) + repmat(old_T, [1, 8]); % 1 is to account for 1-indexing (i.e. start at 0mm)
   ...
   for z = 1:new_dim(3)
      ...
      %
      %  We can use modified new_M1 & old_M1 to substitute new_M & old_M; accounts for -1 index adjustment
      %      new_M1 * new_XYZvox       ==       old_M1 * old_XYZvox
      %
      %  where: M1 = M;   M1(:,4) = M(:,4) - sum(M(:,1:3),2); minus sum across rows same as translation by [-1 -1 -1] vox
      %  and:             M(:,4) == [T; 1] == sum(M1,2)
      %
      %  Therefore:   old_XYZvox = old_M1 \ new_M1 * new_XYZvox; aka inv(old_M1) * (new_M1 * newXYZvox)
      %
      ...
      %  HH: mask_Z ensures that only the z-layer of interest is processed by whichever method below
      %
      ...
      case 2
         new_img(:,:,z) = nearest_neighbor(old_img, new_dim, old_dim, M, bg); % get each z-layer of new image 1 at a time
      case 3
...
function img_slice = nearest_neighbor(img, dim1, dim2, M, bg)
   ...
   for y = 1:ydim1 % these 2 for loops are just iterable breakdown of old_xyz = M*(x; y; 0); z-pos doesn't matter bc col3 empty
      ...
      for x = 1:xdim1
		...
		zi >= 1 & zi <= zdim2	)
            ...
            img_slice(x,y) = img(xi,yi,zi); % new intensity at x, y in z-slice = intensity of nearest voxel in old img

         else
            img_slice(x,y) = bg; % outside boundary just set to background intensity (usually 0)
